...
void DrawLongJump()
{
	if (flJumpmesstime > GetTickCount() && cvar.kz_show_kz)
	{
		...
		if (WorldToScreen(vStart, VecScreenMin) && WorldToScreen(vEnd, VecScreenMax))
			ImGui::GetCurrentWindow()->DrawList->AddLine({ IM_ROUND(VecScreenMin[0]), IM_ROUND(VecScreenMin[1]) }, { IM_ROUND(VecScreenMax[0]), IM_ROUND(VecScreenMax[1]) }, Sakura::Colors::Rainbow());

		if (WorldToScreen(vStart, VecScreenMin))
			ImGui::GetCurrentWindow()->DrawList->AddRectFilled({ IM_ROUND(VecScreenMin[0]) - 1, IM_ROUND(VecScreenMin[1]) - 1 }, { IM_ROUND(VecScreenMin[0]) + 2, IM_ROUND(VecScreenMin[1]) + 2 }, Sakura::Colors::Black());

		if (WorldToScreen(vEnd, VecScreenMax))
			ImGui::GetCurrentWindow()->DrawList->AddRectFilled({ IM_ROUND(VecScreenMax[0]) - 1, IM_ROUND(VecScreenMax[1]) - 1 }, { IM_ROUND(VecScreenMax[0]) + 2, IM_ROUND(VecScreenMax[1]) + 2 }, Sakura::Colors::Black());

		if (WorldToScreen(vStart, VecScreenMin))
		{
			float label_size = IM_ROUND(ImGui::CalcTextSize(/*Start*/XorStr<0xD9, 6, 0xB8B26C33>("\x8A\xAE\xBA\xAE\xA9" + 0xB8B26C33).s, NULL, true).x / 2);
			ImGui::GetCurrentWindow()->DrawList->AddRect({ IM_ROUND(VecScreenMin[0]) - label_size - 2, IM_ROUND(VecScreenMin[1]) - 24 }, { IM_ROUND(VecScreenMin[0]) + label_size + 3, IM_ROUND(VecScreenMin[1]) - 10 }, Sakura::Colors::Rainbow());
			ImGui::GetCurrentWindow()->DrawList->AddText({ IM_ROUND(VecScreenMin[0]) - label_size, IM_ROUND(VecScreenMin[1]) - 25 }, Sakura::Colors::White(), /*Start*/XorStr<0xD9, 6, 0xB8B26C33>("\x8A\xAE\xBA\xAE\xA9" + 0xB8B26C33).s);
		}

		if (WorldToScreen(vEnd, VecScreenMax))
		{
			float label_size = IM_ROUND(ImGui::CalcTextSize(/*Stop*/XorStr<0xFC, 5, 0x204F6DE3>("\xAF\x89\x91\x8F" + 0x204F6DE3).s, NULL, true).x / 2);
			ImGui::GetCurrentWindow()->DrawList->AddRect({ IM_ROUND(VecScreenMax[0]) - label_size - 2, IM_ROUND(VecScreenMax[1]) - 24 }, { IM_ROUND(VecScreenMax[0]) + label_size + 3, IM_ROUND(VecScreenMax[1]) - 10 }, Sakura::Colors::Rainbow());
			ImGui::GetCurrentWindow()->DrawList->AddText({ IM_ROUND(VecScreenMax[0]) - label_size, IM_ROUND(VecScreenMax[1]) - 25 }, Sakura::Colors::White(), /*Stop*/XorStr<0xFC, 5, 0x204F6DE3>("\xAF\x89\x91\x8F" + 0x204F6DE3).s);
		}
	}
}
...
void DrawKzWindows()
{
	if (cvar.kz_show_kz && Sakura::Player::Local::IsAlive())
	{
		ImVec2 windowpos;
		...
		ImGui::Begin(/*kz*/XorStr<0x1C, 3, 0x85263541>("\x77\x67" + 0x85263541).s, NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize);
		{
			...
			ImGui::Text(/*Height:         %.1f*/XorStr<0x47, 21, 0xF990AF3F>("\x0F\x2D\x20\x2D\x23\x38\x77\x6E\x6F\x70\x71\x72\x73\x74\x75\x76\x72\x76\x68\x3C" + 0xF990AF3F).s, HeightOrigin());
			ImGui::Text(/*Ground Angle:   %.1f*/XorStr<0x56, 21, 0x89D13536>("\x11\x25\x37\x2C\x34\x3F\x7C\x1C\x30\x38\x0C\x04\x58\x43\x44\x45\x43\x49\x59\x0F" + 0x89D13536).s, GroundAngle());
